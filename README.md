...
```bash
sudo tar -xf swift-6.1.3-RELEASE-debian-bookworm-armv7-sdk.tar.gz -C /opt
```

Then, from a Swift package, use the `--destination` paramter to cross-compile it for the target:

```bash
swift build --destination /opt/swift-6.1.3-RELEASE-debian-bookworm-armv7/debian-bookworm.json
```
...
```bash
SWIFT_TARGET_ARCH=armv7 ./build-linux-cross-sdk.sh swift-6.1.3-RELEASE ubuntu-noble
```
...
./build-linux-cross-sdk.sh swift-6.1.3-RELEASE ubuntu-noble
```
